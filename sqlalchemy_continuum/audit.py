...
class AuditBase(object):
    ...
    def entity_names(self):
        """
        Return a list of entity names that changed during this audit.
        """
    ...
    def changed_entities(self):
        """
        Return all changed entities for this audit log entry.

        Entities are returned as a dict where keys are entity classes and
        values lists of entitites that changed in this audit.
        """
        ...
        for class_, version_class in tuples:
            ...
            tx_column = manager.option(class_, 'audit_column_name')
...
procedure_sql = """
CREATE OR REPLACE FUNCTION audit_temp_table_generator()
RETURNS TRIGGER AS $$
BEGIN
    {temporary_audit_sql}
    INSERT INTO temporary_audit (id) VALUES (NEW.id);
    RETURN NEW;
END;
$$
LANGUAGE plpgsql
...
def create_triggers(cls):
    sa.event.listen(
        ...
        sa.schema.DDL(
            procedure_sql.format(
                temporary_audit_sql=CreateTemporaryAuditTableSQL(),
                insert_temporary_audit_sql=(
                    InsertTemporaryAuditSQL(
                        audit_id_values='NEW.id'
                    )
                ),
            )
        )
    ...
    sa.event.listen(
        cls.__table__,
        'after_create',
        sa.schema.DDL(str(AuditTriggerSQL(cls)))
    )
    sa.event.listen(
        cls.__table__,
        'after_drop',
        sa.schema.DDL(
            'DROP FUNCTION IF EXISTS audit_temp_table_generator()'
        )
    )
...
class AuditFactory(ModelFactory):
    ...
    def create_class(self, manager):
        ...
        class Audit(
            ...
            id = sa.Column(
                sa.types.BigInteger,
                sa.schema.Sequence('audit_id_seq'),
                primary_key=True,
                autoincrement=True
            )
                ...
                if isinstance(user_cls, six.string_types):
                    try:
                        ...
                        raise ImproperlyConfigured(
                            'Could not build relationship between Audit'
