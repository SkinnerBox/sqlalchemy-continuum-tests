...
from sqlalchemy_continuum.plugins import AuditChangesPlugin
from tests import TestCase


class TestAuditChanges(TestCase):
    plugins = [AuditChangesPlugin()]
...
class TestAuditChangedEntities(TestCase):
    plugins = [AuditChangesPlugin()]
    ...
    def test_saves_only_modified_entity_names(self):
        ...
        AuditChanges = article.__versioned__['audit_changes']

        article.name = u'Some article'
        self.session.commit()

        assert self.session.query(AuditChanges).count() == 1
